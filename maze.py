    ...
    class block:
        def __init__(self,value,right_value,down_value):
            ...
            if self.value == 1:
                ...
                self.dir[3] = False  #self.dir[2] = False
            elif self.value == 3:
                self.dir[0] = False
                self.dir[3] = False  #self.dir[2] = False
        ...
        for j in range(len(L_points[0])):
            ...
            # just one column
                ...
                if i > 0 and i < length - 2:
                    if L_points[i][j].dir[1] == True or L_points[i][j].dir[1] == True:
                        L_gates.append((i,j))
                        if L_points[i][j].dir[1] == True and L_points[i][j].dir[1] == True:
                            L_gates.append((i,j))
        ...
        for i in range(0,len(L_cul)):
            for j in range(0,len(L_cul[0])):
                if L_cul[i][j].value == 1:
                    ...
                    if L_ways[i][j].dir[1] == True and (i,j-1) in L_c:
...
def analyse(self):
    ...
    def display(self):
        ...
        self.file_name=self.file_name.split('.')[0]+'.tex'
        ...
        for i in range(row):
            for j in range(col):
                if walls[i][j].dir[1] == True and walls[i][j].draw_v_row == False:
                    walls[i][j].draw_v_row = True
                    n=j
                    while True:
                        n= n + 1
                        walls[i][n].draw_v_row = True
                        if walls[i][n].dir[1] == False:
                            break
        ...
        L_route_col = []
        for i in L_route:
            if L_ways[i[0]][i[1]].dir[1] == True or L_ways[i[0]][i[1]].dir[1] == True:
        ...
        for i in L_drow_row:
            ...
            if L_ways[i[0][0]][i[0][1]].dir[1] == True:
